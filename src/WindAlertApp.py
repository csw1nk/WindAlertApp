...
ERICKA_PHONE = config['PHONE_NUMBERS']['Ericka']
...
def main_loop():
    ...
    while True:
        if not pause_api_calls:
            ...
            if wind_speed > 22 and not alert_sent:
                message = "Honey, the wind's picking up! :-O Let's roll in the awning! Love you~~~"
                logging.info(f"Alert condition met: Wind speed is {wind_speed} mph. {message}")
                send_sms_via_email(COREY_PHONE, "vtext.com", message)
                send_sms_via_email(ERICKA_PHONE, "vtext.com", message)
                alert_sent = True
